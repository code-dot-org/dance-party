...
test('Dance 2: Fail the level by not changing the move', t => {
  ...
  attempt(userCode, level.validationCode, (result, message) => {
    ...
    t.equals(
      message,
      "Your dancer wasn't doing a new move after the fourth measure."
    );
    t.end();
  });
});
...
test('Dance 7: Fail the level', t => {
  ...
  attempt('', level.validationCode, (result, message) => {
    ...
    t.equals(
      message,
      'Use the `set backup_dancer2 size` block to make that dancer smaller.'
    );
    t.end();
  });
});
...
test('Dance 9: Fail the level', t => {
  ...
  attempt('', level.validationCode, (result, message) => {
    ...
    t.equals(
      message,
      'Try adding the `right_pineapple begins size following bass` block to your program.'
    );
    t.end();
  });
});
...
test('Dance 10: Fail the level', t => {
  ...
  attempt(
    ...
    (result, message) => {
      ...
      t.equals(
        message,
        'Make sure you add a `when key` event and press the key to test it.'
      );
      t.end();
    },
    320
