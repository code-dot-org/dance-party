...
test('can successfully modify song data', t => {
  ...
  t.notEqual(
    songData.analysis[frame].energy[0],
    modified.analysis[frame].energy[0]
  );

  // again pretty weak, but make sure that modifying our params gives us different
  // energy values
  const modifiedSmoothing = modifySongData(
    songDataWithCustomOpts({
      numDeviations: 1.5,
      smoothFactor: 0.5,
    })
  );
  const modifiedDeviations = modifySongData(
    songDataWithCustomOpts({
      numDeviations: 2,
      smoothFactor: 0.7,
    })
  ...
  t.notEqual(
    modified.analysis[frame].energy[0],
    modifiedSmoothing.analysis[frame].energy[0]
  );
  t.notEqual(
    modified.analysis[frame].energy[0],
    modifiedDeviations.analysis[frame].energy[0]
  );
...
test('can specify representativeMeasureRange', t => {
  ...
  const modifiedRepresentativeRange = modifySongData(
    songDataWithCustomOpts({
      numDeviations: 1.5,
      smoothFactor: 0.5,
  ...
  t.notEqual(
    modified.analysis[frame].energy[0],
    modifiedRepresentativeRange.analysis[frame].energy[0]
  );
  t.end();
});
